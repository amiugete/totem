...
$query_insert1 = "INSERT INTO raccolta.effettuati_amiu (
                id,
                tappa, id_causale, 
                datalav, codice, fatto)  
                ( select 
                    (select max(id) from raccolta.effettuati_amiu) + row_number() over (order by cpsxa.id_tappa) as id,
                    cpsxa.id_tappa as tappa, 
                    $1 as causale,
...
$query_insert2 = "INSERT INTO raccolta.percorsi_non_effettuati_x_ekovision (
    id_percorso, datalav,
     causale_sit, codice,
